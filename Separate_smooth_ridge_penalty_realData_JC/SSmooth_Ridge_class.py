...
class SSmooth_Ridge_Opt(BaseEstimator):
    """
    ...
    def fit_ite_solve(self, X, Y, X_dist_mat, Y_dist_mat, X_dis_threshold=17.5, Y_dis_threshold=3):
        """
    ...
    def fit(self, X, Y, X_dist_mat, Y_dist_mat, X_dis_threshold=17.5, Y_dis_threshold=3):
        """
            Using scipy.sparse.linalg.spsolve function to estimate u and v: solve the sparse linear system Ax=b 
            Separate the lambda and alpha, so droping smoothness penalty on cerebellum is easier to apply
            X: cortical matrix
            Y: cerebellumn matrix
            cor_dist_mat: cortical distance matrix
            cere_dist_mat: cerebellumn distance matrix
        ...
        while (r<=self.rank):
            ...
            while (diff_C_norm>1e-3):
                ...
                #u_est, resid1, rank1, s1=LA.lstsq(LA.norm(v_r)**2*self.XTX+self.lambda1*self.L_X, self.X.T@Y_res@v_r,rcond=None)
                start_time1=timeit.default_timer()
                sparse_mat1=scipy.sparse.csr_matrix(LA.norm(v_r)**2*self.XTX+self.lambda_cor*self.L_X+self.alpha_cor*np.eye(self.C_nrow))
                u_est=scipy.sparse.linalg.spsolve(sparse_mat1, self.X.T@Y_res@v_r)
                end_time1=timeit.default_timer()
                dlt_time1=convert(end_time1-start_time1)
                print(f"time used to calulate u_est: {dlt_time1}")
                ...
                #v_est, resid2, rank2, s2=LA.lstsq(u_r@self.XTX@u_r*np.eye(self.ncol)+self.lambda1*self.L_Y, Y_res.T@self.X@u_r, rcond=None)
                start_time2=timeit.default_timer()
                sparse_mat2=scipy.sparse.csr_matrix(u_r.T@self.XTX@u_r*np.eye(self.C_ncol)+self.lambda_cere*self.L_Y+self.alpha_cere*np.eye(self.C_ncol))
                v_est=scipy.sparse.linalg.spsolve(sparse_mat2, Y_res.T@self.X@u_r)
                end_time2=timeit.default_timer()
                dlt_time2=convert(end_time2-start_time2)
                print(f"time used to calulate v_est: {dlt_time2}")
    ...
    def predict(self, X):
        ...
        Predicting
        ----------
        X : array-like or sparse matrix, shape (n_samples, n_cortex_voxels)
            Samples.
       
        Returns: predicted values.
        """
        return np.array(X@self.C_est)    
