...
%             run_suit('SUIT:reslice',s,'contrast', space_label, glm);clc
    ...
    case 'SUIT:vol2surf'                     % STEP 9.9: Make gifti files
        ...
        subj_name=varargin{3}; % 's03' etc
        
        % map volume to surface and save out as gifti
        SUIT_SUBJ_DIR_FUNC = fullfile(BASE_DIR, study, 'suit', glm, subj_name);
        model_names = dir(fullfile(SUIT_SUBJ_DIR_FUNC));

        for m=3:length(model_names)
            
            model_name = model_names(m).name;
            nifti_files = dir(fullfile(SUIT_SUBJ_DIR_FUNC, model_names(m).name));

                for n=3:length(nifti_files), 
                    C=suit_map2surf(fullfile(SUIT_SUBJ_DIR_FUNC, model_name, nifti_files(n).name),'stats','nanmean');

                    % make gifti structure and save out
                    g = gifti(C);
                    out_name = strrep(nifti_files(n).name,'.nii','');
                    out_path = fullfile(SUIT_SUBJ_DIR_FUNC, model_name, strcat(out_name, '.gii'));
                    if ~isfile(out_path)
